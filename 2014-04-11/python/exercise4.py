...
palazzoA = COLOR(RGB([255,222,173])) (palazzoA)

modello = CUBOID([35,7,10])
rotazioneModello = R([1,2]) (PI/2)
modelloRuotato = STRUCT([rotazioneModello,modello])
traslazioneModello = T(1) (7)
modelloTraslato = STRUCT([traslazioneModello,modelloRuotato])
modello = STRUCT([modelloTraslato,modello])
rotazioneModello = R([1,2]) (PI)
modelloRuotato = STRUCT([rotazioneModello,modello])
traslazioneModello = T([1,2]) ([35,35])
modelloTraslato = STRUCT([traslazioneModello,modelloRuotato])
palazzoB = STRUCT([modelloTraslato,modello])

traslaPalazzo = T(2) (30)
palazzoB = STRUCT([traslaPalazzo,palazzoB])
palazzoB = COLOR([0.6,0.4,0.3]) (palazzoB)

figura = STRUCT([figura,palazzoA,palazzoB])
...
strada = CUBOID([5,100,0])
traslazioneStrada = T([1,2]) ([-10,-30])
strada = STRUCT([traslazioneStrada, strada])

striscie = CUBOID([1,3,0])
traslazioneStrisce = T(2) (4)
striscie = STRUCT(NN(24)([traslazioneStrisce, striscie])) 
traslazioneStrada = T([1,2]) ([-8,-32])
striscie = STRUCT([traslazioneStrada, striscie])
striscie = COLOR([5,5,5]) (striscie)

figura = STRUCT([figura,strada,striscie])
